...
# The AWS root takes force_destroy for the same reason: a POC must be removable.
AWS_TF_VARS := -var="region=$(AWS_REGION)"
ifeq ($(POC_DESTROYABLE),true)
AWS_TF_VARS += -var="force_destroy=true"
endif

.PHONY: help setup fmt lint test run docker-build docker-build-aws tf-init tf-plan infra-core image infra deploy-model seed upload-policy destroy clean \
	aws-tf-init aws-tf-plan aws-infra aws-image aws-deploy-endpoint aws-delete-endpoint aws-seed aws-upload-policy aws-destroy
...
aws-tf-plan: ## Plan AWS infrastructure without changing it.
	terraform -chdir=$(AWS_TF_DIR) plan $(AWS_TF_VARS)

aws-infra: ## Create S3, Glue tables, Athena workgroup, ECR, and the runtime IAM role.
	terraform -chdir=$(AWS_TF_DIR) apply $(AWS_TF_VARS)
...
aws-deploy-endpoint: ## Create or update the billable SageMaker endpoint.
	$(eval ECR := $(shell terraform -chdir=$(AWS_TF_DIR) output -raw ecr_repository_url))
	terraform -chdir=$(AWS_TF_DIR) apply $(AWS_TF_VARS) -var="deploy_endpoint=true" -var="container_image=$(ECR):$(AWS_IMAGE_TAG)"

aws-delete-endpoint: ## Tear down the SageMaker endpoint and stop its hourly cost.
	terraform -chdir=$(AWS_TF_DIR) apply $(AWS_TF_VARS) -var="deploy_endpoint=false"
...
aws-destroy: ## Remove every AWS resource created by this POC.
	terraform -chdir=$(AWS_TF_DIR) apply -auto-approve $(AWS_TF_VARS)
	terraform -chdir=$(AWS_TF_DIR) destroy $(AWS_TF_VARS)
